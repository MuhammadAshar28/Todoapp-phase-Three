...
export async function sendMessage(userId: string, request: SendMessageRequest): Promise<APIResponse> {
  ...
  try {
    ...
    // Convert snake_case to camelCase to match APIResponse interface
    const data: APIResponse = {
      conversationId: responseData.conversation_id || responseData.conversationId || '',
      assistantMessage: responseData.assistant_message || responseData.assistantMessage || '',
      success: response.ok,
    };
    ...
    if (typeof data.assistantMessage !== 'string') {
      throw new Error('Invalid response format: assistantMessage is not a string');
    }
